...
public class GeoDynamoDBServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException,
		...
		try {
			...
			} else if (action.equalsIgnoreCase("query-radius-user")) {
				queryRadiusForUser(requestObject, out);
	...
	private void putPoint(JSONObject requestObject, PrintWriter out) throws IOException, JSONException {
		...
		AttributeValue geoMsgKeyAttributeValue = new AttributeValue().withS(requestObject.getString("geoMsg"));
		AttributeValue fromEmailKeyAttributeValue = new AttributeValue().withS(requestObject.getString("fromEmail"));
		AttributeValue fromNameKeyAttributeValue = new AttributeValue().withS(requestObject.getString("fromName"));
		AttributeValue toEmailKeyAttributeValue = new AttributeValue().withS(requestObject.getString("toEmail"));


		PutPointRequest putPointRequest = new PutPointRequest(geoPoint, rangeKeyAttributeValue);
		putPointRequest.getPutItemRequest().addItemEntry("geoMessage", geoMsgKeyAttributeValue);
		putPointRequest.getPutItemRequest().getItem().put("fromEmail", fromEmailKeyAttributeValue);
		putPointRequest.getPutItemRequest().getItem().put("fromName", fromNameKeyAttributeValue);
		putPointRequest.getPutItemRequest().getItem().put("toEmail", toEmailKeyAttributeValue);
	...
	private void printGetPointRequest(GetPointResult getPointResult, PrintWriter out) throws JsonParseException,
		...
		String geoMessage = item.get("geoMessage").getS();
		String fromEmail = item.get("fromEmail").getS();
		String fromName = item.get("fromName").getS();
		String toEmail = item.get("toEmail").getS();
		...
		resultMap.put("geoMessage", geoMessage);
		resultMap.put("fromEmail", fromEmail);
		resultMap.put("fromName", fromEmail);
...
//
	...
	private void queryRadiusForUser(JSONObject requestObject, PrintWriter out) throws IOException, JSONException {
		...
		attributesToGet.add(config.getRangeKeyAttributeName());
		attributesToGet.add(config.getGeoJsonAttributeName());
		attributesToGet.add("geoMessage");
		attributesToGet.add("fromEmail");
		attributesToGet.add("fromName");
		attributesToGet.add("toEmail");
		...
		printGeoQueryResult(queryRadiusResult, userEmail, out);
	}

	private void printGeoQueryResult(GeoQueryResult geoQueryResult, String userEmail, PrintWriter out) throws JsonParseException,
			IOException {
		Map<String, Object> jsonMap = new HashMap<String, Object>();
		...
		for (Map<String, AttributeValue> item : geoQueryResult.getItem()) {
			...
			String toEmail = "";
			if (item.containsKey("toEmail")) {
				toEmail = item.get("toEmail").getS();
			}
			
			if(!toEmail.equals(userEmail))		//Only messages addressed to the user will be returned.
				continue;
			
			String geoMessage = "";
			if (item.containsKey("geoMessage")) {
				geoMessage = item.get("geoMessage").getS();
			}
			
			String fromEmail = "";
			if (item.containsKey("fromEmail")) {
				fromEmail = item.get("fromEmail").getS();
			}
			
			String fromName = "";
			if (item.containsKey("fromName")) {
				fromName = item.get("fromName").getS();
			}
			...
			itemMap.put("geoMessage", geoMessage);
			itemMap.put("fromEmail", fromEmail);
			itemMap.put("fromName", fromName);
