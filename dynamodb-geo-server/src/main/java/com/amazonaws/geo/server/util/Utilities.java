		...
		private void insertData() {
			...
			try {
				while ((line = br.readLine()) != null) {
					...
					String geoMsgId = columns[0];
					String geoMsg = columns[1];
					double latitude = Double.parseDouble(columns[2]);
					double longitude = Double.parseDouble(columns[3]);					
					String fromEmail = columns[4];
					String fromName = columns[4];
					String toEmail = columns[5];

					GeoPoint geoPoint = new GeoPoint(latitude, longitude);
					AttributeValue rangeKeyValue = new AttributeValue().withS(geoMsgId);
					AttributeValue geoMsgValue = new AttributeValue().withS(geoMsg);
					AttributeValue fromEmailValue = new AttributeValue().withS(fromEmail);
					AttributeValue fromNameValue = new AttributeValue().withS(fromName);
					AttributeValue toEmailValue = new AttributeValue().withS(toEmail);

					PutPointRequest putPointRequest = new PutPointRequest(geoPoint, rangeKeyValue);
					putPointRequest.getPutItemRequest().getItem().put("geoMessage", geoMsgValue);
					putPointRequest.getPutItemRequest().getItem().put("fromEmail", fromEmailValue);
					putPointRequest.getPutItemRequest().getItem().put("fromName", fromNameValue);
					putPointRequest.getPutItemRequest().getItem().put("toEmail", toEmailValue);
